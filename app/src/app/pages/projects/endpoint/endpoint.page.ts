    ...
    ngOnInit(): void {
        ...
        this.subscriptions.docs = this.service.documentation.subscribe(docs => {
            if (docs) {
                ...
                for (let a = 0; a < docs.routes.length; a++) {
                    if (docs.routes[a].routeId == this.routeId) {
                        for (let b = 0; b < docs.routes[a].endpoints.length; b++) {
                            if (docs.routes[a].endpoints[b].endpointId == this.endpointId) {
                                found = true;
                                this.endpoint = docs.routes[a].endpoints[b];
                                break;
                            };
                        };
                        break;
